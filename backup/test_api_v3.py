...
def test_api_endpoint():
    ...
    for facility_id in facility_ids:
        ...
        # Try the availability endpoint with the ISO format date
        url = f"https://www.recreation.gov/api/camps/availability/campground/{facility_id}/month?start_date={urllib.parse.quote(month_start + 'T00:00:00.000Z')}"
            ...
            if response.status_code == 200:
                ...
                if 'campsites' in data:
                    ...
                    if data['campsites']:
                        ...
                        # Check what statuses are in the data
                        statuses = set()
                        for site_id, details in data['campsites'].items():
                            for date, status in details.get('availabilities', {}).items():
                                statuses.add(status)
                        print(f"All status types in data: {statuses}")
                else:
